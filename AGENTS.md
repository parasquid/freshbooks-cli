...
- **Blocked release boundary.** If a production release is blocked by CI, workflows, tagging, credentials, or publishing infrastructure, stop and ask. Approval to release a feature is not approval to change release machinery. Explain the failure, risks, and options first, and do not modify release infrastructure unless the user explicitly approves that separate scope.
